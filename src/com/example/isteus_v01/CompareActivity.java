...
public class CompareActivity extends Activity implements SensorEventListener,
		LocationListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String standardPressureString = String
				.valueOf(SensorManager.PRESSURE_STANDARD_ATMOSPHERE);
	...
	protected void onResume() {
		...
		if (enabledProviders.isEmpty()
			...
			Toast.makeText(this, R.string.gpsNotEnabledMessage,
					Toast.LENGTH_LONG).show();
		} else {
			// Register every location provider returned from LocationManager
			for (String provider : enabledProviders) {
				// Register for updates every minute
				locationManager.requestLocationUpdates(provider, 60000, // minimum
																		// time
																		// of
																		// 60000
																		// ms (1
	...
	public void onLocationChanged(Location location) {
		if (LocationManager.GPS_PROVIDER.equals(location.getProvider())
				&& (lastGpsAltitudeTimestamp == -1 || location.getTime()
						- lastGpsAltitudeTimestamp > TIMEOUT)) {
	...
	public void onToggleClick(View view) {
		if (((ToggleButton) view).isChecked()) {
			...
			delta = SensorManager.getAltitude(
					SensorManager.PRESSURE_STANDARD_ATMOSPHERE,
					currentBarometerValue)
	...
	private class MetarAsyncTask extends AsyncTask<Number, Void, Float> {
		private static final String WS_URL = "http://api.openweathermap.org/data/2.5/weather?q=Antalya,tr";
		...
		protected Float doInBackground(Number... params) {
			...
			try {
				...
				Uri uri = Uri.parse(WS_URL).buildUpon()
				...
				String observation = json.getJSONObject("weatherObservation")
				...
				mslp = (Math.abs((1000 - val1)) < Math.abs((1000 - val2))) ? val1
						: val2;
			} catch (Exception e) {
